      ...
      <section className="panel mt-5 overflow-hidden" aria-label="Lista de cidades">
        ...
        {filteredCities.map((city) => {
          ...
          return <article key={city.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-2 border-b border-white/[.075] px-4 py-4 last:border-0 hover:bg-white/[.025] lg:grid-cols-[minmax(220px,1.5fr)_150px_130px_112px] lg:gap-4 lg:px-5">
            <div className="min-w-0"><div className="flex items-center gap-3"><div className="flex shrink-0 flex-row gap-1"><button type="button" className="grid h-7 w-7 place-items-center rounded-md text-white/55 transition-colors hover:bg-white/[.07] hover:text-gold disabled:cursor-not-allowed disabled:opacity-25" aria-label={`Mover ${city.name} para cima`} disabled={cityHasPendingCrud || hasAnyPendingReorder || cityIndex <= 0} onClick={() => void moveCity(city.id, "up")}><ArrowUp size={14} /></button><button type="button" className="grid h-7 w-7 place-items-center rounded-md text-white/55 transition-colors hover:bg-white/[.07] hover:text-gold disabled:cursor-not-allowed disabled:opacity-25" aria-label={`Mover ${city.name} para baixo`} disabled={cityHasPendingCrud || hasAnyPendingReorder || cityIndex < 0 || cityIndex >= orderedCities.length - 1} onClick={() => void moveCity(city.id, "down")}><ArrowDown size={14} /></button></div><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-[#d4af37]/30 bg-[#d4af37]/10 text-gold"><MapPinned size={18} /></span><h2 className="truncate font-semibold text-white">{city.name}</h2></div></div>
