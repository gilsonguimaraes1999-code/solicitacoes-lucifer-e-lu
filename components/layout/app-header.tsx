...
import type { EffectivePermissions, Profile } from "@/features/requests/types";

export function AppHeader({ profile, permissions }: { profile: Profile; permissions: EffectivePermissions }) {
  return (
    <header className="border-b border-white/10 bg-black/60 px-4 py-3 backdrop-blur-xl md:px-6">
      <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-4">
        ...
        <nav className="flex items-center gap-2 text-sm">
          ...
          {permissions.canManageCities && <Link href="/admin/cities" className="nav-link"><MapPinned size={16} /><span className="hidden sm:inline">Cidades</span></Link>}
